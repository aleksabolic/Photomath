...
def mathml2latex_yarosh(equation):
    ...
    cleaned_equation = ''.join(c for c in equation if 31 < ord(c) < 127)

    dom = etree.fromstring(cleaned_equation)
    xslt = etree.parse(xslt_file)
    transform = etree.XSLT(xslt)
    newdom = transform(dom)
    
    # remove garabage from the output
    newdom = str(newdom).replace('\\backslash ', '\\').replace("\\mathrm", "").replace(" ", '')
...
def _call_mathml_cloud(equation, mathtype):
    ...
    try:
        ...
        sys.stderr.write('MathML Cloud ERROR: %sn idk')
        sys.exit(2)
